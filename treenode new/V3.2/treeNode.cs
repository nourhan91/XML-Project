 ...
 * V 3.2
 *
 * printing :bug fixed if the tag name is one char
 * parsing :bug fixed if the tag attribute name is one char
 * parsing :bug fixed print "[" if any after the attributes
 *
 *
 */
///\\\
    ...
    class treeNode
    {
        ...
        public void xmlparser(ref List<string> xml)
        {
            ...
            while (index < xml.Count())
            {
                ...
                // value or tag with one char name
                else
                {
        ...
        static public void Main(string[] args)
        {
            ...
            /*string xml = "pointer refs=\"n05616246 n05200169\" source=\"1\" target=\"1\"";            string[] words = xml.Split(' ');            int i = xml.IndexOf('=');*/
            //int apperacne = xml[index].ToString().Count(zx => zx == '=');
            /*foreach (var word in words)
            {            int equal = word.IndexOf('=');                if (equal>=0)               {
                    string before = word.Substring(0, equal);
                    int bef = word.IndexOf('=') + 1;                    //string asd=xml.Substring(xml.EndsWith(before))
                    string x=xml.Substring(bef, xml.Count() - before.Count());
